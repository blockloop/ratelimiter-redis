...
import (
	...
	"github.com/blockloop/ratelimiter"
...
func limitTestFunc(r *miniredis.Miniredis, l ratelimiter.Limit) func(t *testing.T) {
	return func(t *testing.T) {
		limiter := NewRedisLimiter(&fakePool{addr: r.Addr()}, []ratelimiter.Limit{l})
		ip := uuid.New()
...
func TestLimiterCallsOnError(t *testing.T) {
	...
	ip := uuid.New()
	limiter := NewRedisLimiter(p, []ratelimiter.Limit{l})
